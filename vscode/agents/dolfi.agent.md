...
6. **Validate** — check the markup is well-formed SVG per the SVG 2 spec, renders correctly at target sizes, and visually matches sibling icons (same weight, same grid, same style). If you need a visual confirmation of the rendered result (optical alignment, legibility, consistency), delegate the icon image to 🐙 **Chululu** for visual validation.
